...
import {
  ...
  INSIGNIA_BOOK_CP_TABLE, KARAZAD_BASE_STAT,
  FAMILY_ITEMS
...
function renderInsigniaSection(root) {
  const card = buildCard("문양 각인서 (1회성)",
    "구매+감정 한 번으로 끝나는 소모품입니다(강화 반복 없음). 감정 결과가 등급별 구간 내 무작위라 공격력·방어력 중앙값의 합을 전투력 근사로 사용합니다.");
  card.appendChild(buildTable("등급", [{ label: "전투력(1회)", key: "v" }],
    [{ label: "심연", key: "심연" }, { label: "태고", key: "태고" }, { label: "혼돈", key: "혼돈" }],
    function (r) { return INSIGNIA_BOOK_CP_TABLE[r.key][0]; }));
  root.appendChild(card);
}
