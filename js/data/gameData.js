...
export const RECOVERY_NOTES = {
  accessory: "실패해도 단계가 하락하지 않고, 은화 또는 돌파 복구권으로 100% 복구됩니다.",
  relic: "실패 시 은화 또는 돌파 복구권으로 복구됩니다.",
  harmonyAlchemy: "실패 시 은화로 복구됩니다(0~4강 45,000, 이후 5단계마다 45,000씩 증가).",
  lightstone: "실패 시 은화 또는 돌파 복구권으로 복구됩니다.",
  riftTotem: "실패 시 0→1강은 복구 비용 없음, 1~7강은 결정화된 토템, 8·9강은 오기에르의 가호 1개로 100% 복구됩니다."
};
...
// 밤·달빛 영혼석: 등급 없이 단일 곡선, 자동 계산. 최고 단계는 13강까지입니다(사용자 확인).
export const SOUL_ITEMS = [
  { id: "nightsoul", name: "밤의 영혼석", material: "밤의 영혼석 강화 재료" },
  { id: "moonsoul", name: "달빛 영혼석", material: "달빛 영혼석 강화 재료" }
];

// 밤·달빛 영혼석 강화 성공확률은 전 단계 51%로 동일합니다. 실패할 때마다 두 선택지가 있습니다
// (사용자 확인, 2026-08-20): (A) 그 단계 확정 복구 비용(SOUL_RECOVERY_COST[단계], 강화 재료
// 개수)을 내고 그 단계에 남거나, (B) 결제하지 않고 위험을 감수 — 그 단계 "복구 재료"를 이미
// 갖고 있으면 공짜로 그 단계에 남고, 없으면 0강으로 완전히 떨어지되 그 단계 복구 재료를 얻습니다
// (재료는 단계마다 독립적으로 쌓이고 다음에 그 단계에서 실패할 때 공짜로 씀, 절대 안 사라짐).
export const SOUL_RECOVERY_COST = [0, 1, 3, 6, 11, 19, 34, 59, 102, 175, 301, 518, 890];
export const SOUL_SUCCESS_RATE = 0.51;
export const SOUL_MAX_STEP = 13;

// 위 두 선택지를 매 실패마다 최적으로 골랐을 때(값 반복/벨만 방정식으로 오프라인 계산,
// scripts/soul_solver.mjs 참고 — 상태공간이 커서(목표 13강 기준 8,192가지 복구재료 보유
// 조합 × 13단계) 브라우저에서 매번 풀면 1초 넘게 걸려 미리 계산해 둡니다) 0강부터 각 목표
// 단계까지 드는 재료 기대 개수. 배열 인덱스 = 목표 단계.
export const SOUL_OPTIMAL_CUMULATIVE_QTY = [
  0, 1.960784, 4.882353, 9.72549, 17.45098, 29.905674, 49.008705, 77.59012,
  120.057874, 183.385203, 277.576836, 417.447185, 624.950214, 932.601523
];

// 단계별로 "결제"와 "위험 감수" 중 어느 쪽이 최적인지(13강을 최종 목표로 잡고 푼 결과 기준,
// 아직 그 단계 복구 재료를 하나도 못 가진 상태에서의 선택). 0강→1강만 복구 비용이 0이라 결제가
// 당연히 유리하고(사실상 선택의 의미가 없음), 그 외 전 구간은 위험 감수가 압도적으로 유리합니다
// — 재료를 한 번 모아두면 이후 그 단계를 다시 지날 때마다 계속 공짜로 방어해주기 때문입니다.
export const SOUL_RECOVERY_POLICY = ["pay", "risk", "risk", "risk", "risk", "risk", "risk", "risk", "risk", "risk", "risk", "risk", "risk"];
...
export const GEAR_PRESETS = [
  ...
  },
  {
    ...
    relic: { grade: "혼돈", level: 5, awakened: false },
    extra: {
  ...
  },
  {
    ...
    desc: "공허 장비 +8 · 공허 장신구 +9 · 장비·장신구 각성 완료",
    equip: { grade: "공허", step: 8, awakened: true },
    accessory: { grade: "공허", level: 9, awakened: true },
    relic: { grade: "혼돈", level: 8, awakened: true },
    extra: {
  ...
  },
  {
    ...
    desc: "공허 장비 +9 이상 · 카라자드 장신구 · 장비·장신구 각성 완료",
    equip: { grade: "공허", step: 9, awakened: true },
    accessory: { grade: "카라자드", level: 0, awakened: true },
    relic: { grade: "공허", level: 8, awakened: true },
    extra: {
