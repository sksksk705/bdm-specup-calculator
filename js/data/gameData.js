...
// 밤·달빛 영혼석 강화 성공확률은 전 단계 51%로 동일하고, 실패할 때마다 그 단계에서 확정 복구
// 하는 데 드는 강화 재료 개수가 배열 인덱스(=강화 단계, 0강→1강이 인덱스 0)로 정해져 있습니다
// (사용자 확인, 2026-08-20 — 이전에는 "실패하면 0강으로 완전히 떨어지되 복구 재료를 1개 받고,
// 이미 갖고 있으면 즉시 복구"라는 확률적 방식으로 알고 있어 몬테카를로 시뮬레이션을 썼지만,
// 실제로는 이렇게 매 실패마다 확정 비용을 내는 방식이라 닫힌 공식(기하분포)으로 정확히 계산
// 가능합니다). 최고 단계는 13강.
